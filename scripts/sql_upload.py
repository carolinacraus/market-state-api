...
from scripts.logger import get_logger  # ✅ Import your existing logger
...
def upload_market_state_to_api(system_name: str) -> dict:
    ...
    try:
        ...
        logger.exception("❌ Failed to read local diagnostics file")
        return {"error": f"Failed to read local diagnostics file: {e}"}

    if "date" not in df.columns or "direction" not in df.columns:
        logger.error("❌ Required columns missing in diagnostics file")
